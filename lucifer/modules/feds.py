...
from lucifer import dispatcher, OWNER_ID, SUDO_USERS, WHITELIST_USERS, MESSAGE_DUMP, LOGGER
...
def new_fed(bot: Bot, update: Update):
    ...
    if not fednam == '':
        ...
        if not x:
            update.effective_message.reply_text("Failed to create federation! Head over to @lucifertm to notify us of the error.")
            return
...
def join_fed(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
                message.reply_text("Failed to join federation! Please head to @lucifertm to report this.")
                return
...
def fed_ban(bot: Bot, update: Update, args: List[str]):
    ...
    if fban:
        ...
        if not x:
            message.reply_text("Failed to ban from the federation! If this problem persists, reach out to us @lucifertm.")
            return
...
def set_frules(bot: Bot, update: Update, args: List[str]):
    ...
    if len(args) >= 1:
        ...
        if not x:
            update.effective_message.reply_text("Failed to set federation rules. If this persists, reach out to us @lucifertm.")
            return
    ...
    if args:
        if args[0] == 'json':
            ...
            with BytesIO(str.encode(backups)) as output:
                output.name = "lucifer_fbanned_users.json"
                update.effective_message.reply_document(document=output, filename="lucifer_fbanned_users.json",
            ...
            for users in getfban:
                ...
                backups += "\n"
            with BytesIO(str.encode(backups)) as output:
                output.name = "lucifer_fbanned_users.csv"
                update.effective_message.reply_document(document=output, filename="lucifer_fbanned_users.csv",
