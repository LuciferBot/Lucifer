...
from lucifer import dispatcher, OWNER_ID, LYDIA_API
...
from lucifer.modules.helper_funcs.misc import sendMessage
...
from lucifer import dispatcher,WALL_API
import random
...
from lucifer import dispatcher, CASH_API_KEY
...
import datetime
from lucifer import dispatcher, TIME_API_KEY, LOGGER
...
def lydia(bot: Bot, update: Update):
    ...
    if msg.text and not msg.document:
        ...
        try:
            ...
            bot.send_message(MESSAGE_DUMP, f"Lydia error: {e} occurred in {chat_id}!")
...
def wall(bot: Bot, update: Update, args):
    ...
    if not query:
        ...
        if not json_rep.get("success"):
            msg.reply_text("An error occurred! Report this @luciferbot")
        else:
...
def slap(bot: Bot, update: Update, args: List[str]):
    ...
    if user_id == bot.id:
        temp = random.choice(memes_strings.SLAP_lucifer_TEMPLATES)
...
Misc = " \
 ...
 \n - /feedback : You can give us your feedbacks  \
   an can see your feeds @luciferbotfeeds here. \
